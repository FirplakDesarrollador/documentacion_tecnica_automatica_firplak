...
(async () => {
    try {
        const { data, error } = await (supabase as any).rpc('execute_sql_internal', { sql_query: sql });
        if (error) {
            console.error('Error:', error);
            // Si falla, intentamos otra vía: usar el cliente para una operación que fuerce el error pero nos deje ver si conecta
            console.log('Trying direct query via postgres-node (if you have it)...');
        } else {
