...
async function fixTrigger() {
    console.log("Fixing dual-write trigger (removing 'riel')...");
    const ddl = `
    ...
    BEGIN
        ...
        INSERT INTO public.product_skus
            (version_id, sku_complete, sap_description_original, final_name_complete_es, final_name_complete_en, status)
        VALUES
        ...
        ON CONFLICT (sku_complete) DO UPDATE SET
            ...
            final_name_complete_es = EXCLUDED.final_name_complete_es,
            final_name_complete_en = EXCLUDED.final_name_complete_en,
            status = EXCLUDED.status;
    ...
    try {
        await dbQuery(ddl);
        console.log("Trigger fixed (removed 'riel').");
    } catch (e: any) {
