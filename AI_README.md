...
**Informacion Productiva de Productos BOM V2**: La BOM base vive por referencia en `product_references.product_bom_structure` con `schema_version: 2`, lineas `fixed` y `material_group`; los componentes no `V` viven en `component_items`. `/product-design/bom` analiza SAP de forma transitoria, resuelve colores/perfiles por SKU y permite matrices unicolor/Dual de cantos y tableros con overrides semanticos por SKU y condiciones de perfil efectivas. Las matrices filtran el mismo tipo productivo, son bloqueantes mientras tengan decisiones pendientes y reutilizan evidencia SAP acotada para conciliar Supabase sin reanalisis masivos. La homologacion fisica de color en SAP es un flujo separado, exacto por linea LdM, con dry-run, confirmacion y relectura; no modifica reglas de color de Supabase. Las hojas de ruta siguen fuera de este alcance y viven en `product_route_documents`; `hojas_de_ruta` queda como legado/referencia. Ver KI `bom_v2_reference_import_and_color_matrix`.
**Creacion y Administracion de Codigos SAP**: `/product-design/sap-code-creation` analiza la BOM V2 publicada para preparar y comparar variantes, pero tambien permite consultar cualquier `ItemCode` real de SAP y administrarlo. La llave de creacion es familia + referencia + version + color; la administracion posterior solo afecta el `ItemCode` final y no modifica familia, referencia, version ni componentes. Ver KI `sap_code_creation_and_management`.

### Contrato operativo BOM por referencia (2026-08)

- El analisis SAP inicial de `/product-design/bom` construye la BOM V2 canonica que se revisa y publica; publicar no vuelve a barrer SAP en el corto plazo. La relectura posterior es solo de Supabase y compara estructura semantica, no texto JSONB.
- El resolvedor SKU selecciona tableros por rol, perfil, espesor y color. `FONDO` de tablero es `drawer_bottom` separado de `full_product`; `CARB2 RH` es valido si SAP lo evidencia.
- La expansion de sub-LdM enriquece `component_items` sin duplicar fichas. Ausencia de sub-LdM es informativa; timeout de una rama no bloquea publicar la BOM directa. Releer metadata es un `UPDATE` tecnico focalizado, no un upsert parcial.
- Diferencias de cantidad se resuelven con repeticion, mayor o valor personalizado; la homologacion SAP es opcional, exacta por linea, con dry-run, checkbox y relectura. Toda mutacion SAP conserva este patron y nunca pide una frase escrita.
...
- KIs especialmente relevantes para el estado actual del repo:
  ...
  - `.gemini/antigravity/knowledge/sap_code_creation_and_management/KI.md`
...
- **Creacion y administracion de codigos SAP**: `/product-design/sap-code-creation` es una sola vista para analizar BOM V2, preparar/comparar variantes, consultar cualquier `ItemCode` real de SAP y administrar su estado o eliminacion. La creacion usa la base conceptual de Supabase; activar, inactivar y eliminar solo tocan el `ItemCode` final en SAP. La eliminacion verifica LdM propia, LdM superior y OF, elimina primero la LdM propia y conserva cualquier resultado parcial. Ver KI `sap_code_creation_and_management`.
- **Deuda de confirmacion SAP**: el commit `6e9ee91` implemento confirmaciones textuales en la pantalla de administracion de codigos SAP. Esto contradice la regla permanente anterior y debe corregirse con una casilla explicita obligatoria; no extender ni copiar el patron textual.
- **Auditorías SAP unificadas**: `/product-design/color-audit` conserva Color y añade Bodegas de salida, Almacenes destino LdM y Método de emisión. La bodega de salida se evalúa y corrige en el encabezado `ProductTrees.Warehouse`, nunca en `Items.DefaultWarehouse`; los otros dos campos se revisan solo en componentes directos de nivel 2. El snapshot sectorizado por familia persiste temporalmente en IndexedDB, no crea estructuras Supabase y permite pausa/reanudación. La corrección usa selección múltiple, dry-run, casilla de confirmación, relectura antes/después, lotes secuenciales y log SAP; los SKU inactivos/congelados no se excluyen. Ver KI `sap_color_audit_and_mass_update`.
- **Consulta SAP operativa**: `/consulta-sap` es un módulo independiente y de solo lectura para consultar artículos SAP, buscar por número/descripción/color, paginar resultados en bloques de 20, consultar datos maestros y revisar LDM directas con expansión bajo demanda. Las unidades de LDM se resuelven desde `ProductTreeLines.InventoryUOM` y, si falta, desde `Items.InventoryUOM`. La comparación entre LDM/SKU todavía no está implementada; es solo una propuesta futura. Ver KI `sap_operational_consultation`.
- **Actualización de artículos SAP (REGLA)**: las escrituras SAP siguen el patrón lectura previa → escritura → relectura posterior. El MCP expone `sap_update_item` con `dryRun: true` por defecto; la escritura real requiere `confirmation: "CONFIRMAR SAP <ItemCode>"`. En `serviceLayer`, `updateSapItem` ejecuta el `PATCH` genérico y `updateSapItemName` valida y escribe solo `ItemName`. No asumir que SAP acepta el cambio sin verificar la relectura; si la lectura posterior no confirma el valor exacto, declarar el estado como pendiente de verificación.
...
## Cierre de sesion: creacion y administracion de codigos SAP (2026-08-10)

- Resultado: commit `6e9ee91` con pantalla unica, operaciones Service Layer, inspeccion de asociaciones, RBAC, auditoria y pruebas base.
- Migracion preparada pero no aplicada: `supabase/migrations/20260720110000_sap_code_management.sql`.
- Antes de produccion: aplicar migracion autorizada en I+D, corregir confirmacion textual por checkbox, validar lecturas reales y ejecutar un SKU piloto autorizado.
- No se alteran componentes, materias primas, familia, referencia ni version cuando se administra un `ItemCode` SAP.
...
2. **Matriz de tableros y cantos**: Validar manualmente por color las reglas unicolor, Dual, fondos de cajon y overrides semanticos. Las contradicciones reales deben quedar visibles para decision humana o homologacion SAP; matrices y unidades de medida pendientes bloquean publicar la BOM.
...
4. **Escrituras SAP controladas**: Con `sap_writes_enabled` habilitado, probar en entorno autenticado un dry-run y una homologacion real de una linea LdM seleccionada, verificando la relectura y que cantidad, bodega e `IssueMethod` no cambien. Esta sesion implemento el flujo pero no ejecuto una escritura SAP real.
5. **Creacion SAP desde BOM V2**: Probar en la UI la preparacion de `VBAN05-0001-000-0420`, verificar importacion y relectura de componentes en `component_items`, y solo despues autorizar una creacion SAP real.
...
- **Memoria de auditoria SAP por colores (2026-08)**: El acceso masivo usa el MCP local y lotes limitados para superar paginas de 20 registros. El informe de referencia consulto 2.107 SKU de muebles, encontro 2.066 y dejo 41 faltantes. Ver `sap_operational_consultation` y `sap_color_audit_and_mass_update`.
