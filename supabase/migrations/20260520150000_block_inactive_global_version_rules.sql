-- If a global version rule is INACTIVO, any product using that version_code is not exportable.
...
COMMENT ON VIEW public.v_ui_generate_list IS 'READ-ONLY UI MODEL: Overrides + status efectivo (incluye bloqueo por global_version_rules INACTIVO).';
