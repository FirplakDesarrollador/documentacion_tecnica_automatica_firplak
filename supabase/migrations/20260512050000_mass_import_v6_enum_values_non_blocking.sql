-- Mass Import V6: allow new enum values (warning) instead of blocking.
--
-- Rules:
-- - REF_ATTR key must exist in families.ref_attrs_schema for the family (otherwise dropped with warning).
-- - If schema type=enum and value maps to allowed_values -> store canonical allowed value (warning if normalized).
-- - If schema type=enum and value does NOT map -> store raw value and emit warning (no block).
-- - If schema enum is malformed (missing allowed_values) -> still ERROR (schema misconfiguration).
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      -- REF_ATTR governance + enums with "accept new values" behavior.
        ...
        FOR v_attr_key, v_val IN SELECT * FROM jsonb_each(v_ref_attrs_clean) LOOP
          ...
          IF v_type = 'enum' THEN
            ...
            IF v_mapped IS NULL THEN
              -- Accept but warn. Keep the raw value so the system can evolve naturally.
              v_warns := array_append(v_warns, format('Valor "%s" no está en allowed_values para "%s" (familia "%s"). Se guardará como nuevo valor; considera actualizar el schema.', v_val #>> '{}', v_attr_key, v_family_code));
            ELSE
