-- Cotizador integrado de nuevos productos V1.
-- Aplicada en Supabase I+D como migración remota 20260811113936 el 2026-08-11.
--
-- Esta fuente canónica conserva la estructura aplicada: una estimación editable
-- previa a la creación formal del producto y una toma de Ingeniería por muestra.
-- No crea artículos, códigos, versiones ni LdM en SAP.
--
-- La cotización conserva snapshots de LdM, costos, geometría, factores y muestras
-- en draft_data_json. No se crea una tabla de revisiones ni una tabla puente de
-- muestras: el snapshot evita que una cotización ya guardada cambie al ingresar
-- nuevas mediciones.
...
-- RLS no expone acceso directo. Las rutas/actions del servidor aplican
-- assertPermission y usan service_role para Diseño, Ingeniería y Ventas.
...
-- Carga histórica inicial: sólo las nueve filas REAL del libro de costeo.
-- CALCULO 3D, PROMEDIO y comparaciones individuales de OASIS no se importan.
-- El localizador archivo/hoja/fila y DO NOTHING hacen que el seed sea idempotente
-- sin sobrescribir enriquecimientos manuales posteriores.
...
VALUES
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'LVM OSLO 48X38 (REAL)', 'LVM OSLO 48X38 (REAL)',
    2270000, 6.2, 355800, 0.4,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B3', 'cad_volume_mm3', 'C3', 'mixture_kg', 'D3',
        'paint_area_mm2', 'F3', 'gelcoat_kg', 'G3'
      ),
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'LVM VESSEL DOHA (REAL)', 'LVM VESSEL DOHA (REAL)',
    1439000, 4, 257700, 0.4,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B5', 'cad_volume_mm3', 'C5', 'mixture_kg', 'D5',
        'paint_area_mm2', 'F5', 'gelcoat_kg', 'G5'
      ),
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'Meson Oasis 63x48 (REAL)', 'Meson Oasis 63x48 (REAL)',
    1928000, 6, 279000, 0.46,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B7', 'cad_volume_mm3', 'C7', 'mixture_kg', 'D7',
        'paint_area_mm2', 'F7', 'gelcoat_kg', 'G7'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'Lavamanos Meca Vessel 35x35 (REAL)', 'Lavamanos Meca Vessel 35x35 (REAL)',
    1850000, 4, 238200, 0.37,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B9', 'cad_volume_mm3', 'C9', 'mixture_kg', 'D9',
        'paint_area_mm2', 'F9', 'gelcoat_kg', 'G9'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'LVR VERSA 55X50 - REAL', 'LVR VERSA 55X50 - REAL',
    3416000, 9.5, 482700, 0.6,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B16', 'cad_volume_mm3', 'C16', 'mixture_kg', 'D16',
        'paint_area_mm2', 'F16', 'gelcoat_kg', 'G16'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'Lavarropas Versa 50X50 - REAL', 'Lavarropas Versa 50X50 - REAL',
    3131000, 8, 432900, 0.42,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B17', 'cad_volume_mm3', 'C17', 'mixture_kg', 'D17',
        'paint_area_mm2', 'F17', 'gelcoat_kg', 'G17'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'MESON COCINA VERSA 150X55 BLANCO FORT-SIN PERF CUB-SIN PERF GRIF (REAL)',
    'MESON COCINA VERSA 150X55 BLANCO FORT-SIN PERF CUB-SIN PERF GRIF (REAL)',
    6596000, 19.8, 1055000, 1.4,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B20', 'cad_volume_mm3', 'C20', 'mixture_kg', 'D20',
        'paint_area_mm2', 'F20', 'gelcoat_kg', 'G20'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'MESON COCINA ECO 150X55 BLANCO FORT-SIN PERF CUB-SIN PERF GRIF (REAL)',
    'MESON COCINA ECO 150X55 BLANCO FORT-SIN PERF CUB-SIN PERF GRIF (REAL)',
    9743000, 20, 1078000, 1.45,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B24', 'cad_volume_mm3', 'C24', 'mixture_kg', 'D24',
        'paint_area_mm2', 'F24', 'gelcoat_kg', 'G24'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
  ...
  (
    'SYNTHETIC_MARBLE_GENERAL', 'valid',
    'MESON COCINA ECO 150X55 BLANCO FORT-PERF CUB-SIN PERF GRIF (REAL)',
    'MESON COCINA ECO 150X55 BLANCO FORT-PERF CUB-SIN PERF GRIF (REAL)',
    7468000, 23, 1053000, 1.5,
    'historical_excel',
    ...
    jsonb_build_object(
      ...
      'cell_ranges', jsonb_build_object(
        'label', 'B26', 'cad_volume_mm3', 'C26', 'mixture_kg', 'D26',
        'paint_area_mm2', 'F26', 'gelcoat_kg', 'G26'
      ),
      'formula_cells', '{}'::jsonb,
      'read_mode', 'evaluated_cell_result'
    )
...
-- Verificación posterior obligatoria (fuera de la migración):
-- 1) Deben existir nueve filas valid para SYNTHETIC_MARBLE_GENERAL.
-- 2) Sus totales deben ser: volumen=37841000 mm3, mezcla=100.5 kg,
--    área=5232300 mm2 y gelcoat=7 kg.
-- 3) Factores por razón de totales esperados:
--    mezcla=0.0000026558494754366955 kg/mm3 y
--    gelcoat=0.0000013378437780708292 kg/mm2.
-- 4) RLS debe estar activo, sin privilegios directos para anon/authenticated,
--    y service_role debe conservar SELECT/INSERT/UPDATE/DELETE.
--
-- Reversión: no eliminar registros productivos de forma automática. Antes de
-- revertir código o esquema, exportar ambas tablas y verificar sus conteos. Con
-- autorización explícita, eliminar primero los triggers y funciones y después
-- las tablas; las FKs usan SET NULL y no deben afectar el catálogo ni SAP.
