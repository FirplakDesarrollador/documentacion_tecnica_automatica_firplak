...
export default function AcceptRecoveryPage() {
  ...
  useEffect(() => {
    ...
    const acceptRecovery = async () => {
      ...
      if (!accessToken || !refreshToken) {
        if (active) {
          setError(recoveryError || 'El enlace de recuperación no es válido o ya expiró. Solicita uno nuevo a un administrador.')
        }
        return
      }
      ...
      if (sessionError) {
        if (active) {
          setError('No se pudo validar el enlace de recuperación. Solicita uno nuevo a un administrador.')
        }
        return
      }
      ...
      <div className="relative z-10 w-full max-w-md px-6 py-12 text-center">
        ...
        {error ? (
          <div className="rounded-2xl border border-red-500/20 bg-white p-8 shadow-premium">
            ...
            <h1 className="text-xl font-bold text-primary">No se pudo abrir el enlace</h1>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-premium">
            ...
            <h1 className="mt-4 text-xl font-bold text-primary">Validando acceso</h1>
            <p className="mt-2 text-sm text-slate-600">Te llevaremos a actualizar tu contraseña.</p>
          </div>
        )}
      </div>
    </div>
