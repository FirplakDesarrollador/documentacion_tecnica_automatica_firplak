    ...
    <div className="flex flex-col gap-6">
      ...
      {establishedRefId && selectedReference ? (
        <div ref={workspaceRef} className="flex flex-col gap-4">
          ...
          <ContrastPanel referenceId={draft.source.reference_id} isApproved={status === 'approved'} />
...
function ContrastPanel({ referenceId, isApproved }: { referenceId: string | null; isApproved: boolean }) {
  const [report, setReport] = useState<{
    ...
    coverage_issues: Array<{ type: string; sku_complete: string; item_name: string | null }>
  } | null>(null)
  ...
  async function handleContrast() {
    ...
    setReport(null)
    try {
      const result = await contrastVsSapAction(referenceId)
      if (result.error) setError(result.error)
      else setReport(result.report)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error al contrastar')
    } finally {
      setLoading(false)
    }
  }

  const badgeClass = (match: string) => {
    if (match === 'full') return 'bg-emerald-50 text-emerald-700 ring-emerald-200'
    if (match === 'error') return 'bg-rose-50 text-rose-700 ring-rose-200'
    return 'bg-amber-50 text-amber-700 ring-amber-200'
  }

  const lineBadgeClass = (match: string) => {
    if (match === 'exact') return 'bg-emerald-50 text-emerald-700 ring-emerald-200'
    if (match === 'qty_mismatch') return 'bg-amber-50 text-amber-700 ring-amber-200'
    if (match === 'sap_only') return 'bg-blue-50 text-blue-700 ring-blue-200'
    return 'bg-purple-50 text-purple-700 ring-purple-200'
  }

  const lineLabel = (match: string) => {
    if (match === 'exact') return 'Coincide'
    if (match === 'qty_mismatch') return 'Cant. distinta'
    if (match === 'sap_only') return 'Solo SAP'
    if (match === 'app_only') return 'Solo App'
    return match
  }
  ...
  return (
    <SectionCard
      title="Contraste SAP"
      description="Compara la BOM resuelta por la aplicacion contra lo que realmente tiene SAP para cada SKU de esta referencia."
    >
      {isApproved ? (
        <button
          type="button"
        ...
        >
          {loading ? 'Contrastando...' : 'Contrastar vs SAP'}
        </button>
      ) : (
        <p className="text-xs text-slate-500">Disponible solo cuando la ruta esta aprobada.</p>
      )}
      ...
      {report ? (
        <div className="mt-4 space-y-3">
          <div className="flex flex-wrap gap-3 text-xs">
            <span className="rounded-md bg-slate-100 px-2 py-1 font-semibold">Total SKUs: {report.total_skus}</span>
            <span className="rounded-md bg-emerald-100 px-2 py-1 font-semibold text-emerald-800">100%: {report.full_match_count}</span>
            <span className="rounded-md bg-amber-100 px-2 py-1 font-semibold text-amber-800">Diff: {report.difference_count}</span>
            {report.error_count > 0 ? (
              <span className="rounded-md bg-rose-100 px-2 py-1 font-semibold text-rose-800">Error: {report.error_count}</span>
            ) : null}
          </div>

          {report.sku_results.map((sku) => {
            const isExpanded = expandedSku === sku.sku_complete
            return (
              <div key={sku.sku_complete} className="rounded-md border border-slate-200">
                <button
                  type="button"
                  onClick={() => setExpandedSku(isExpanded ? null : sku.sku_complete)}
                  className="flex w-full items-center justify-between px-3 py-2 text-left text-xs font-semibold hover:bg-slate-50"
                >
                  <span>{sku.sku_complete}</span>
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ring-1 ${badgeClass(sku.match)}`}>
                    {sku.match === 'full' ? '100%' : sku.match === 'error' ? 'Error' : `${sku.differences.length} diff`}
                  </span>
                </button>
                {isExpanded ? (
                  <div className="border-t border-slate-100 px-3 py-2">
                    {sku.error ? (
                      <p className="text-xs text-rose-600">{sku.error}</p>
