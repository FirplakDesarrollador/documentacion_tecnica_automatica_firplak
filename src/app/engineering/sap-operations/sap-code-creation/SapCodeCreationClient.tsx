...
import { type SapItemTargetStatus } from '@/lib/sap/itemLifecycle'
...
export default function SapCodeCreationClient({ canManageSapCodes }: { canManageSapCodes: boolean }) {
  ...
  const [componentSyncConfirmed, setComponentSyncConfirmed] = useState(false)
    ...
    async function loadInitialData() {
      try {
        const [referencesResponse, colorsResponse] = await Promise.all([
          fetch('/api/engineering/sap-operations/sap-code-creation'),
          fetch('/api/engineering/sap-operations/sap-code-creation/colors'),
        ])
  ...
  async function postAction(body: Record<string, unknown>): Promise<ApiResponse> {
    const response = await fetch('/api/engineering/sap-operations/sap-code-creation', {
      method: 'POST',
  ...
  async function runCreation(action: 'prepare' | 'compare' | 'create') {
    ...
    try {
      const payload = await postAction({
        ...
        barcodeIntent,
        barcodeValue,
        confirmed: action === 'create' && createConfirmed,
      ...
      if (action === 'prepare') {
        ...
        setComponentSyncConfirmed(false)
      } else {
  ...
  async function syncMissingComponents() {
    if (!prepared || !componentSyncConfirmed) return
    ...
    try {
      ...
      if (payload.success !== true) throw new Error(getErrorMessage(payload, 'No se pudieron sincronizar los componentes.'))
      setComponentSyncConfirmed(false)
      await runCreation('prepare')
      setActionMessage(typeof payload.message === 'string' ? payload.message : 'Componentes sincronizados; se actualizó la validación.')
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'No se pudieron sincronizar los componentes.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function searchItems(append: boolean) {
    ...
    try {
      ...
      const response = await fetch(`/api/engineering/sap-operations/sap-code-creation/search?${params.toString()}`)
  ...
  async function runStatus(targetStatus: SapItemTargetStatus, dryRun: boolean) {
    ...
    try {
      const payload = await postAction({
        ...
        targetStatus,
        dryRun,
        confirmed: dryRun ? false : statusConfirmed,
      })
      if (dryRun) {
  ...
  async function runDelete(dryRun: boolean) {
    ...
    try {
      const payload = await postAction({
        ...
        itemCode,
        dryRun,
        confirmed: dryRun ? false : deleteConfirmed,
      })
      if (dryRun) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 text-slate-900 sm:p-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-5">
        <div>
          <Link href="/engineering" className="text-sm font-semibold text-indigo-600 hover:text-indigo-800">← Ingeniería</Link>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {prepared && !prepared.existing && prepared.context.missing.length === 0 && canManageSapCodes ? <button disabled={busy || !createConfirmed} onClick={() => void runCreation('create')} className="rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Crear Item + LdM en SAP</button> : null}
          </div>
          {prepared && !prepared.existing && prepared.context.missing.length === 0 && canManageSapCodes ? (
            <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              <label className="flex items-start gap-2 font-semibold">
                <input type="checkbox" checked={createConfirmed} onChange={event => setCreateConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4" />
                Confirmo crear en SAP el artículo y su LdM exactamente como fueron preparados.
              </label>
            </div>
          ) : null}
          {prepared?.context.missing.length ? (
            <div className="mt-3 rounded-md bg-rose-50 p-3 text-sm text-rose-700">
              <p>Faltantes: {prepared.context.missing.join(', ')}</p>
              {canManageSapCodes ? (
                <div className="mt-3 rounded-md border border-rose-200 bg-white p-3 text-slate-700">
                  <p className="text-xs">Preparar y validar no modifica el catálogo de componentes. La sincronización es una acción separada.</p>
                  <label className="mt-2 flex items-start gap-2 text-xs font-semibold">
                    <input type="checkbox" checked={componentSyncConfirmed} onChange={event => setComponentSyncConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4" />
                    Confirmo sincronizar estos componentes desde SAP al catálogo de Supabase.
                  </label>
                  <button type="button" disabled={busy || !componentSyncConfirmed} onClick={() => void syncMissingComponents()} className="mt-3 rounded-md border border-indigo-600 px-3 py-2 text-xs font-semibold text-indigo-700 disabled:opacity-50">
                    Sincronizar componentes y volver a validar
                  </button>
                </div>
              ) : null}
            </div>
          ) : null}
          {prepared?.context.warnings.map(warning => <p key={warning} className="mt-2 text-xs text-amber-700">Aviso: {warning}</p>)}
        </section>
        ...
        {inspection ? <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 text-sm md:grid-cols-3">
            <div className="rounded-md bg-slate-50 p-3"><span className="text-xs text-slate-500">Valid</span><p className="font-semibold">{itemValue(inspection.item, 'Valid') || 'No informado'}</p></div>
            <div className="rounded-md bg-slate-50 p-3"><span className="text-xs text-slate-500">Frozen</span><p className="font-semibold">{itemValue(inspection.item, 'Frozen') || 'No informado'}</p></div>
          ...
          <div className="mt-5 grid gap-5 lg:grid-cols-2">
            <div className="rounded-lg border border-slate-200 p-4">
              <h3 className="font-semibold">LdM propia</h3>
              {inspection.bom ? <><p className="mt-1 text-sm text-slate-600">{inspection.bom.treeType || 'Tipo no informado'} · {inspection.bom.lineCount} líneas.</p><div className="mt-3 max-h-72 overflow-auto"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-slate-100"><tr><th className="p-2">Componente</th><th className="p-2">Cantidad</th><th className="p-2">Almacén</th></tr></thead><tbody>{inspection.bom.lines.map((line, index) => <tr key={`${line.ItemCode}-${index}`} className="border-t border-slate-100"><td className="p-2 font-mono">{line.ItemCode}</td><td className="p-2">{line.Quantity}</td><td className="p-2">{line.Warehouse || '—'}</td></tr>)}</tbody></table></div></> : <p className="mt-1 text-sm text-slate-500">No tiene LdM propia en SAP.</p>}
            </div>
            <div className="rounded-lg border border-slate-200 p-4">
              <h3 className="font-semibold">Asociaciones detectadas</h3>
              {inspection.associations.warnings.map(warning => <p key={warning} className="mt-2 text-xs text-amber-700">Aviso: {warning}</p>)}
              {inspection.associations.parentTrees.length === 0 && inspection.associations.productionOrders.length === 0 && inspection.associations.complete ? <p className="mt-2 text-sm text-emerald-700">No se detectaron asociaciones superiores ni OF.</p> : null}
              {inspection.associations.parentTrees.map(tree => <p key={`tree-${tree.treeCode}`} className="mt-2 text-sm text-rose-700">LdM superior: <span className="font-mono">{tree.treeCode}</span></p>)}
              {inspection.associations.productionOrders.map(order => <p key={`order-${order.absoluteEntry ?? order.documentNumber}`} className="mt-2 text-sm text-rose-700">OF: {order.documentNumber ?? order.absoluteEntry ?? 'sin número'} · {order.status || 'estado no informado'}</p>)}
            </div>
          </div>

          {canManageSapCodes ? (
            <div className="mt-5 rounded-lg border border-indigo-200 bg-indigo-50/50 p-4">
              <h3 className="font-semibold text-slate-900">Administrar este código</h3>
              <p className="mt-1 text-xs text-slate-600">Las acciones afectan únicamente a <span className="font-mono">{inspection.itemCode}</span>. No modifican familia, referencia, versión ni componentes.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                <button type="button" disabled={actionLoading} onClick={() => void runStatus('ACTIVO', true)} className="rounded-md border border-emerald-600 bg-white px-3 py-2 text-sm font-semibold text-emerald-700 disabled:opacity-50">Dry-run activar</button>
                <button type="button" disabled={actionLoading} onClick={() => void runStatus('INACTIVO', true)} className="rounded-md border border-amber-600 bg-white px-3 py-2 text-sm font-semibold text-amber-700 disabled:opacity-50">Dry-run inactivar</button>
                <button type="button" disabled={actionLoading} onClick={() => void runDelete(true)} className="rounded-md border border-rose-600 bg-white px-3 py-2 text-sm font-semibold text-rose-700 disabled:opacity-50">Dry-run eliminar</button>
              </div>
              {statusPreview && statusTarget ? <div className="mt-3 rounded-md border border-slate-200 bg-white p-3 text-sm"><p>{statusPreview.message}</p><label className="mt-2 flex items-start gap-2 font-semibold"><input type="checkbox" checked={statusConfirmed} onChange={event => setStatusConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4" />Confirmo actualizar el estado de este código en SAP.</label><button type="button" disabled={actionLoading || !statusConfirmed} onClick={() => void runStatus(statusTarget, false)} className="mt-2 rounded-md bg-slate-900 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Confirmar {statusTarget === 'ACTIVO' ? 'activación' : 'inactivación'}</button></div> : null}
              {deletePreview ? <div className="mt-3 rounded-md border border-rose-200 bg-white p-3 text-sm"><p>{deletePreview.message}</p><p className="mt-1 text-xs text-slate-600">LdM propia: {String((deletePreview.plan as { treeLineCount?: number } | undefined)?.treeLineCount ?? 0)} líneas · asociaciones verificadas: {String((deletePreview.plan as { associationCheckComplete?: boolean } | undefined)?.associationCheckComplete ?? false)}</p><label className="mt-2 flex items-start gap-2 font-semibold"><input type="checkbox" checked={deleteConfirmed} onChange={event => setDeleteConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4" />Confirmo eliminar este código y su LdM propia en SAP.</label><button type="button" disabled={actionLoading || !deleteConfirmed} onClick={() => void runDelete(false)} className="mt-2 rounded-md bg-rose-700 px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Confirmar eliminación</button></div> : null}
            </div>
          ) : <p className="mt-5 rounded-md bg-slate-50 p-3 text-sm text-slate-600">Tu perfil puede consultar este artículo, pero no tiene habilitada la administración de códigos SAP.</p>}
        </section> : null}
        ...
        {fallbackAvailable && inspection ? <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900"><p>La eliminación física fue bloqueada por SAP. Puedes inactivar únicamente este ItemCode.</p><button type="button" disabled={actionLoading} onClick={() => void runStatus('INACTIVO', true)} className="mt-2 rounded-md border border-amber-600 bg-white px-3 py-2 text-xs font-semibold text-amber-700 disabled:opacity-50">Preparar inactivación alternativa</button></div> : null}
