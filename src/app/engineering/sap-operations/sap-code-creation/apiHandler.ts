...
import { updateSapItemStatusAction } from '@/app/engineering/sap-operations/sap-code-creation/actions'
import {
...
type CreationRequest = {
  action: 'prepare' | 'compare' | 'create' | 'inspect' | 'status' | 'delete' | 'sync-components'
  referenceId: string
  versionId: string
  colorCode: string
...
function parseRequest(value: unknown): CreationRequest {
  ...
  const supportedActions = new Set<CreationRequest['action']>(['prepare', 'compare', 'create', 'inspect', 'status', 'delete', 'sync-components'])
...
async function executeSapItemDeletion(input: CreationRequest) {
  ...
  if (!input.confirmed) {
    throw new Error('Marca la confirmación explícita antes de eliminar el artículo de SAP.')
  }
...
async function buildContext(input: CreationRequest): Promise<CreationContext> {
  ...
  let componentSync: SapComponentCatalogSyncResult = { importedItemCodes: [], unavailableItemCodes: [], missingInSapItemCodes: [], errors: [] }
  let physicalBoardSync: SapComponentCatalogSyncResult = { importedItemCodes: [], unavailableItemCodes: [], missingInSapItemCodes: [], errors: [] }

  if (input.action === 'sync-components') {
    componentSync = await syncMissingSapComponentsToCatalog(
      resolvedLines
    ...
    physicalBoardSync = await syncPhysicalBoardCandidatesToCatalog(
      resolvedLines
...
export async function postSapCodeCreation(request: Request) {
  ...
  try {
    ...
    const isMutation = input.action === 'create' || input.action === 'status' || input.action === 'delete' || input.action === 'sync-components'
    if (isMutation) {
    ...
    if (input.action === 'sync-components' && !input.confirmed) {
      throw new Error('Marca la confirmación explícita antes de sincronizar componentes en Supabase.')
    }
    ...
    if (input.action === 'sync-components') {
      return NextResponse.json({
        success: true,
        mode: 'sync-components',
        importedItemCodes: context.componentSync.importedItemCodes,
        unavailableItemCodes: context.componentSync.unavailableItemCodes,
        missingInSapItemCodes: context.componentSync.missingInSapItemCodes,
        errors: context.componentSync.errors,
        message: 'Sincronización explícita finalizada. Vuelve a preparar para validar la LdM actualizada.',
      })
    }
    ...
    if (!input.confirmed) {
      throw new Error('Marca la confirmación explícita antes de crear el artículo y la LdM en SAP.')
    }
    await assertSapWritesEnabled()
