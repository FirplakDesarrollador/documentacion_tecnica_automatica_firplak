...
async function main() {
    const lines = await prisma.$queryRaw`SELECT DISTINCT line FROM public.products WHERE line IS NOT NULL AND line != ''`;
    ...
    if (Array.isArray(dirty) && dirty.length > 0) {
        ...
        for (const c of manualColors) {
            await prisma.$executeRawUnsafe(`UPDATE public.products SET line = NULL WHERE UPPER(line) LIKE '%${c}%'`);
        }
