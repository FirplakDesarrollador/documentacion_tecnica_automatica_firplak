...
const sql = `
-- 1. Cambiar tipo de columna private_label_client_id a text
ALTER TABLE public.cabinet_products 
ALTER COLUMN private_label_client_id TYPE text USING private_label_client_id::text;

-- 2. Añadir columna special_label con valor por defecto 'NA'
ALTER TABLE public.cabinet_products 
ADD COLUMN IF NOT EXISTS special_label text DEFAULT 'NA';

-- 3. Asegurar que special_label sea 'NA' para los existentes
UPDATE public.cabinet_products SET special_label = 'NA' WHERE special_label IS NULL OR special_label = '';

-- 4. Mover 'ESPECIAL OBRA' a special_label y limpiar client_name
...
-- 5. Normalizar private_label_client_name a 'NA' para los NULL o vacíos
UPDATE public.cabinet_products 
SET private_label_client_name = 'NA' 
WHERE private_label_client_name IS NULL OR private_label_client_name = '';

-- 6. Asignar IDs únicos y activar flags para clientes conocidos
UPDATE public.cabinet_products 
